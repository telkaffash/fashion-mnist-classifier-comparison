# Load a saved SVM and evaluate on the test set.
#
# Usage:
#   python svm/predict.py --feature raw
#   python svm/predict.py --feature hog
#   python svm/predict.py --feature pca

import sys, os
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

import argparse
import config
from utils.dataPreprocessing import load_features, load_labels
from utils.helperFunctions   import load_svm, print_results, plot_confusion_matrix
from svm.train               import predict_svm

MODELS_DIR = os.path.join(os.path.dirname(__file__), "results", "models")
PLOTS_DIR  = os.path.join(os.path.dirname(__file__), "results", "plots")


def parse_args():
    p = argparse.ArgumentParser()
    p.add_argument("--feature", default="pca", choices=["raw", "hog", "pca"])
    return p.parse_args()


def main():
    args = parse_args()

    print(f"Loading {args.feature} features ...")
    X_test = load_features("test", args.feature)
    y_test = load_labels("test")

    weights = load_svm(os.path.join(MODELS_DIR, f"svm_{args.feature}.pkl"))
    y_pred  = predict_svm(X_test, weights)

    print_results(y_test, y_pred, model_name=f"SVM [{args.feature}]")
    plot_confusion_matrix(y_test, y_pred,
                          title=f"SVM [{args.feature}]",
                          save_path=os.path.join(PLOTS_DIR, f"cm_{args.feature}_eval.png"))


if __name__ == "__main__":
    main()